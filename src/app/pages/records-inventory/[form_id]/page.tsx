    ...
    function reducer(state: State, action: Action): State {
        switch (action.type) {
            ...
            case 'update_field': {
                ...
                console.log('Updating items field', state)
                return state.map((group) =>
                    group.id === groupId
                        ? {
                            ...group,
            ...
            case 'update_group_value_field': {
                ...
                return state.map((group) => {
                    ...
                    const updatedGroupValues = group.group_values.map((item, idx) => {
                        ...
                        console.log('update_group_value_field >>', state)
                        const updatedItem = {
                            ...item,
                            [field]: value,
                            ...(field === 'time_value' && value === "P"
        ...
        <Layout className='relative'>
            <Layout.Header sticky >
                <Breadcrumb>
                    <BreadcrumbList>
                        <BreadcrumbItem>
                            <BreadcrumbLink href="/pages/records-inventory/">Records-inventory</BreadcrumbLink>
                        </BreadcrumbItem>
                        <BreadcrumbSeparator />
                        <BreadcrumbItem>
                            <BreadcrumbPage>{form_id}</BreadcrumbPage>
                        </BreadcrumbItem>
                    </BreadcrumbList>
                </Breadcrumb>
            </Layout.Header>
            ...
            <Layout.Body >
                <div className="flex flex-row gap-5 p-2">
                    <Button className="" variant="outline">
                        <Link className="flex flex-row gap-2 items-center" href="/pages/records-inventory">
                    ...
                    <Button
                        ...
                        variant='outline'>
                        <IconRefresh size={14} /> <p>Refresh</p>
                    </Button>
                    <Button
                        disabled={isLoadingFormData}
                        className="flex flex-row gap-[7px] items-center justify-center"
                        variant="outline"
                        onClick={submitForm}
                    >
                    ...
                    <Button className='flex flex-row gap-1' disabled={isLoadingFormData} onClick={downloadAsPDF} variant='outline'>
                        <IconFileTypePdf size={14} /> <p>Preview</p>
                    </Button>
                </div>
                <ResizablePanelGroup
                        ...
                        <div className="flex flex-col gap-5 min-w-fit ">
                            <div className="flex flex-row gap-3 items-start justify-start border-solid border-bottom border-blue-500 ">
                                <div className="flex flex-wrap gap-2">
                                    <p className='font-bold'>Records Series</p>
                                    {isLoadingFormData ?
                                        ...
                                        <div key={idx} className={cn(is_editing && items?.length && 'border-primary', items?.length ? '' : 'border-destructive', 'flex flex-col gap-3 p-3 rounded-lg border-solid border border-solid border-[2px]  mr-3 min-w-[140px]')}>
                                            <div className='w-full flex flex-row gap-2 items-center'>
                                                <Button
                                                    ...
                                                    className='text-green-600 hover:text-green-600' variant='ghost'>
                                                    {
                                                ...
                                                <div className="flex flex-col gap-2 w-full">
                                                    <Label>Group title ({items?.length})</Label>
                                                    <Input
                                                        ...
                                                        value={group_title}
                                                        className=''
                                                        placeholder='Disabled when single unit' />
                                                </div>

                                                <Button className='px-0 text-red-600 hover:text-600' variant='ghost'
