...
import NewUsersDialog from "./new-user-dialog"
        ...
        <div className="flex flex-col ">
            <div className='flex justify-between items-center'>
                <div className="flex flex-rwo gap-2 items-center px-3">
                    ...
                    <p className="dark:bg-gray-800 px-2 rounded-md bg-gray-100 text-gray-600 text-[13px]">{totalUsers} users</p>
                </div>
                <div className="flex gap-3 items-center p-3 border-b border-b-[1px] border-b-secondary">
                    <Input
                        placeholder="Offline search for email"
                        ...
                        className="max-w-sm"
                    />
                    <Button onClick={() => fetchUsers()} variant='outline' className='p-1 gap-2'><IconRefresh size={18} />Refresh</Button>
                    <NewUsersDialog />
                            ...
                            !isloading ? <>{table.getRowModel()?.rows?.length ? (
                                table.getRowModel().rows.map((row) => (
                                    ...
                                    >
                                        {row.getVisibleCells().map((cell) => (
                                            <TableCell key={cell.id} className="max-w-[200px] truncate whitespace-nowrap overflow-hidden text-ellipsis" >
                                ...
                                <>
                                    {table.getRowModel()?.rows?.map((item, index) => (
                                        <TableRow className="p-0 m-0" key={index}>
                                            {
